...
def generate_tex_table(data, names):
    with open('table.tex', 'w') as myfile:
        ...
        for i in range(len(names)):
            ...
            myfile.write('& $\dadi$ grid 1 & '+"%.2e"%dt[0, 0]+' & '+"%.2e"%dt[0, 1]
                         +' & '+"%.2e"%dt[0, 2]+' & '+"%.2e"%dt[0, 3]+' & '+str(dt[0, 4])+' \\\ \n ')
            myfile.write(names[i]+' & $\dadi$ extrapolation & '+"%.2e"%dt[1, 0]+' & '+"%.2e"%dt[1, 1]
                         +' & '+"%.2e"%dt[1, 2]+' & '+"%.2e"%dt[1, 3]+' & '+str(dt[1, 4])+' \\\ \n ')
            myfile.write(' & \\textit{Moments} & '+"%.2e"%dt[2, 0]+' & '+"%.2e"%dt[2, 1]
                         +' & '+"%.2e"%dt[2, 2]+' & '+"%.2e"%dt[2, 3]+' & '+str(dt[2, 4])+' \\\ \n \hline \n')
        # end of the table
