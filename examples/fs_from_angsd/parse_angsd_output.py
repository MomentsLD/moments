...
def make_parser():
    ...
    parser.add_argument('--sfs_file', '-i', required=True,
                        help="n-dimensional SFS from ANGSD")
    parser.add_argument('--pop_file', '-p', type=str, required=True,
                        help="File with population labels for samples")
    parser.add_argument('--pop_ids', type=str, nargs='+', required=True,
                        help="List of populations ids, matching order of ANGSD output")
    parser.add_argument('--folded', '-f', action="store_false",
    ...
    parser.add_argument('--mask_corners', action="store_false",
...
def plot_fs(fs):
    ...
    resonable.
    """
    assert moments.__version__ >= '1.0.3', "Plotting function relies on changes made in\
