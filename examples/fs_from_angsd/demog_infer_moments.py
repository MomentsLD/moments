...
def dog_demog(params, ns):
    ...
    fs.integrate([Nb_gl, N_wolf, N_cn], T_present,
            m=np.array([[0, 0, 0], [m_wolf_gl, 0, 0], [0, 0, 0]]))

    return fs

def get_fs_from_angsd(sfs_file,
                      pop_file,
                      pop_ids=['GL_DOG', 'CN_DOG', 'ARCTIC_WOLF'],
                      fold=False):
    """
...
def to_understandable_params(params, theta, mu, g):
    ...
    (N_anc_dog, N_wolf, N_cn, N_gl, Nb_gl,
     Tsplit_gl_cn, Tb_gl, T_present,
     m_wolf_gl) = params
    ...
    params = (N_anc_dog, N_wolf, N_cn, N_gl, Nb_gl,
              Tsplit_gl_cn, Tb_gl, T_present,
              m_wolf_gl)
...
#    print(f"{np.log(p0)}")
    ...
    xopt = moments.Inference.optimize_log_powell(
            p0, data_fs, dog_demog, lower_bound=lower, upper_bound=upper,
            verbose=1, flush_delay=2/60, maxiter=5,
            fixed_params = fixed)
