...
dd = moments.Misc.make_data_dict("data.txt")

# Extract the spectrum for ['YRI','CEU'] from that dictionary, with both
# projected down to 20 samples per population.
fs = moments.Spectrum.from_data_dict(dd, ["YRI", "CEU"], [20, 20])

# Let's plot the fs.
import pylab

moments.Plotting.plot_single_2d_sfs(fs, vmin=0.1)
pylab.show()

# If we didn't have outgroup information, we could use the folded  version
# of the fs.
folded = moments.Spectrum.from_data_dict(dd, ["YRI", "CEU"], [20, 20], polarized=False)
...
Q = moments.Numerics.array_from_file("Q.HwangGreen.human.dat")
...
moments.Misc.make_fux_table("fux_table.dat", 0.0112, Q, tri_freq)
# And finally we get the corrected frequency spectrum.
fs_corr = moments.Spectrum.from_data_dict_corrected(
    dd, ["YRI", "CEU"], [20, 20], "fux_table.dat"
)
