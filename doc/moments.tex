...
$\Phi_n (i) = \int  {n \choose i} x^i (1-x)^{n-i}\phi(x)$dx
...
$$\Phi^{\mathbf{n}}_{\mathbf{i}}= \int \prod_{j=1}^p { n_j \choose i_j} x_j^{i_j} (1-x_j)^{n_j-i_j} \phi(\mathbf{x})dx_j  $$ where $\mathbf{n}$ is a vector of sample sizes and $\mathbf{i}$ is a vector of counts of alternates for $P$ populations.   
