...
\begin{equation}
\begin{split}
	\dot \Phi_n(i) &= n u \delta_{i=1} + \frac{1}{4 N} \int_0^1 w_i \pd{}{x}\left( x(1-x) \phi(x,t)\right)dx \\
...
One can derive a similar set of equations taking into account selection and dominance either by adapting the rational used for the neutral case, or by projecting the partial differential equation on the weigh functions.
\eqref{eq:selection}:
\begin{equation}
\begin{split}
\dot \Phi_n(i)=& nu  \delta_{i=1} + \frac{1}{4 N} \left[ (i-1)(n-i+1) \Phi_n(i-1)\delta_{i\geq 2} \right.\\
