...
\end{figure}
 ...
 \section{Alternative introduction of the method}
...
\paragraph{Heterozygosity rate evolution}
...
\begin{equation*}
\begin{split}
\Phi_n^{k+1}(i) =& 2n\mu \delta_{i=1} + (1-\frac{\delta_{1\leq i\leq n-1}}{2N})\Phi_n^{k}(i) \\
...
\begin{equation}
\begin{split}
\Phi_n^{k+1}(i)=& \Phi_n^{k}(i) + 2n\mu \delta_{i=1} + \frac{1}{4 N} \left[ (i-1)(n-i+1) \Phi_n^{k}(i-1)\delta_{i\geq 2} \right.\\
...
\paragraph{Adding selection and dominance}


The same expression can be derived from the diffusion equation projecting it on the binomiale weight functions $w_i=  {n\choose i}  x^i (1-x)^{n-i}$. The approach is explained in appendix \ref{calc}.
