...
nu1 = .5 # relative size of population 1
...
population_configurations = [
    msprime.PopulationConfiguration(sample_size=ns[0], 
        initial_size=nu1*Ne),
    msprime.PopulationConfiguration(sample_size=ns[1], 
        initial_size=nu2*Ne)]

migration_matrix = [[0, m], [m, 0]]

demographic_events = [
    msprime.MassMigration(time=gens, source=1, 
        destination=0, proportion=1),
    msprime.MigrationRateChange(time=gens, rate=0),
    msprime.PopulationParametersChange(time=gens, 
        initial_size=Ne, population_id=0)]

ts = msprime.simulate(Ne=Ne, length=L, 
    recombination_rate=r, mutation_rate=u,
...
ld_stats = mold.Parsing.compute_ld_statistics(
    'data/two_pop.vcf',
    rec_map_file = 'data/rec_map.txt', map_name = 'map',
    pop_file = 'data/samples.txt', pops = ['pop1', 'pop2'],
    cM = False,
