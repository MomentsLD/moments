...
def corrected_pi(y, ns=None, num_pops=None):
    if num_pops == None or ns == None:
        ...
        return LDstats(A.dot(y), num_pops=1, basis='pi', order=2)
    elif num_pops == 2:
        A = pi_corrections_2pop(ns)
        return LDstats(A.dot(y), num_pops=2, basis='pi', order=2)
    elif num_pops == 3:
        A = pi_corrections_3pop(ns)
        return LDstats(A.dot(y), num_pops=3, basis='pi', order=2)
    elif num_pops == 4:
...
def corrected_pi_genotypes(y, ns=None, num_pops=None):
    if num_pops == None or ns == None:
        ...
        A = pi_corrections_genotype_2pop(ns)
        return LDstats(A.dot(y), num_pops=2, basis='pi', order=2)
    elif num_pops == 3:
        A = pi_corrections_genotype_3pop(ns)
        return LDstats(A.dot(y), num_pops=3, basis='pi', order=2)
    elif num_pops == 4:
...
def pi_corrections_1pop(ns):
    n1 = ns[0]
    row = [ 0, 0, 0, 1, 1, 2, 2, 2, 3, 4, 5 ]
    col = [ 0, 1, 2, 0, 1, 0, 1, 2, 3, 4, 5 ]
    data = [ (-2. + 4.*n1 - 3.*n1**2 + n1**3)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (-1.*n1 + n1**2)/n1**3, (8. - 12.*n1 + 4.*n1**2)/n1**3, (-4. + 8.*n1 - 5.*n1**2 + n1**3)/n1**3, (-2. + 2.*n1)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (n1 - 2.*n1**2 + n1**3)/n1**3, (-1. + n1)/n1, (-1. + n1)/n1, 1 ]
    return csc_matrix((data,(row,col)),shape=(6,6))

def pi_corrections_2pop(ns):
    n1,n2 = ns
    row = [ 0, 0, 0, 1, 2, 2, 2, 3, 3, 4, 5, 6, 6, 7, 7, 8, 9, 10, 10, 11, 11, 11, 12, 12, 13, 14, 14, 15, 15, 15, 15, 16, 16, 17, 18, 18, 19, 19, 19, 20, 21, 22, 23, 24, 25, 26 ]
    col = [ 0, 3, 11, 1, 2, 10, 19, 0, 3, 4, 5, 1, 6, 1, 7, 8, 9, 2, 10, 0, 3, 11, 4, 12, 13, 5, 14, 1, 6, 7, 15, 8, 16, 17, 9, 18, 2, 10, 19, 20, 21, 22, 23, 24, 25, 26 ]
    data = [ (-2. + 4.*n1 - 3.*n1**2 + n1**3)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (-1.*n1 + n1**2)/n1**3, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (-2. + 4.*n2 - 3.*n2**2 + n2**3)/n2**3, (1. - 2.*n2 + n2**2)/n2**3, (-1.*n2 + n2**2)/n2**3, (8. - 12.*n1 + 4.*n1**2)/n1**3, (-4. + 8.*n1 - 5.*n1**2 + n1**3)/n1**3, (2. - 3.*n1 + n1**2)/n1**2, (2. - 3.*n1 + n1**2)/n1**2, (-4. + 4.*n1)/(n1*n2), (-1.*n2 + n1*n2)/(n1*n2), (-4. + 4.*n2)/(n1*n2), (-1.*n1 + n1*n2)/(n1*n2), (2. - 3.*n2 + n2**2)/n2**2, (2. - 3.*n2 + n2**2)/n2**2, (8. - 12.*n2 + 4.*n2**2)/n2**3, (-4. + 8.*n2 - 5.*n2**2 + n2**3)/n2**3, (-2. + 2.*n1)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (n1 - 2.*n1**2 + n1**3)/n1**3, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, 1/(n1*n2), 0.25/n1, 0.25/n2, 1., (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (-2. + 2.*n2)/n2**3, (1. - 2.*n2 + n2**2)/n2**3, (n2 - 2.*n2**2 + n2**3)/n2**3, (-1. + n1)/n1, 1., (-1. + n2)/n2, (-1. + n1)/n1, 1., (-1. + n2)/n2, 1 ]
    return csc_matrix((data,(row,col)),shape=(27,27))

def pi_corrections_3pop(ns):
    n1,n2,n3 = ns
    row = [ 0, 0, 0, 1, 2, 3, 3, 3, 4, 5, 5, 5, 6, 6, 7, 8, 9, 10, 10, 11, 12, 13, 14, 14, 15, 15, 16, 17, 18, 19, 19, 20, 21, 22, 23, 23, 24, 24, 25, 26, 27, 28, 28, 29, 30, 31, 32, 32, 33, 33, 33, 34, 34, 35, 35, 36, 37, 38, 39, 39, 40, 40, 40, 40, 41, 41, 42, 42, 43, 43, 44, 45, 45, 46, 46, 47, 47, 47, 47, 48, 49, 49, 50, 50, 51, 52, 52, 53, 54, 54, 54, 55, 55, 56, 57, 58, 58, 59, 59, 60, 60, 61, 61, 61, 61, 62, 62, 63, 64, 65, 65, 66, 67, 67, 68, 68, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81 ]
    col = [ 0, 6, 33, 1, 2, 3, 19, 54, 4, 5, 32, 68, 0, 6, 7, 8, 9, 1, 10, 11, 12, 13, 2, 14, 1, 15, 16, 17, 18, 3, 19, 20, 21, 22, 4, 23, 2, 24, 25, 26, 27, 4, 28, 29, 30, 31, 5, 32, 0, 6, 33, 7, 34, 8, 35, 36, 37, 38, 9, 39, 1, 10, 15, 40, 11, 41, 16, 42, 17, 43, 44, 12, 45, 13, 46, 2, 14, 24, 47, 48, 25, 49, 26, 50, 51, 18, 52, 53, 3, 19, 54, 20, 55, 56, 57, 21, 58, 27, 59, 22, 60, 4, 23, 28, 61, 29, 62, 63, 64, 30, 65, 66, 31, 67, 5, 32, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81 ]
    data = [ (-2. + 4.*n1 - 3.*n1**2 + n1**3)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (-1.*n1 + n1**2)/n1**3, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (1. - 1.*n1 - 1.*n3 + n1*n3)/(n1*n3), (-2. + 4.*n2 - 3.*n2**2 + n2**3)/n2**3, (1. - 2.*n2 + n2**2)/n2**3, (-1.*n2 + n2**2)/n2**3, (1. - 1.*n2 - 1.*n3 + n2*n3)/(n2*n3), (-2. + 4.*n3 - 3.*n3**2 + n3**3)/n3**3, (1. - 2.*n3 + n3**2)/n3**3, (-1.*n3 + n3**2)/n3**3, (8. - 12.*n1 + 4.*n1**2)/n1**3, (-4. + 8.*n1 - 5.*n1**2 + n1**3)/n1**3, (2. - 3.*n1 + n1**2)/n1**2, (2. - 3.*n1 + n1**2)/n1**2, (2. - 3.*n1 + n1**2)/n1**2, (-4. + 4.*n1)/(n1*n2), (-1.*n2 + n1*n2)/(n1*n2), (-1. + n1)/n1, (2. - 3.*n1 + n1**2)/n1**2, (-1. + n1)/n1, (-4. + 4.*n1)/(n1*n3), (-1.*n3 + n1*n3)/(n1*n3), (-4. + 4.*n2)/(n1*n2), (-1.*n1 + n1*n2)/(n1*n2), (2. - 3.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (2. - 3.*n2 + n2**2)/n2**2, (8. - 12.*n2 + 4.*n2**2)/n2**3, (-4. + 8.*n2 - 5.*n2**2 + n2**3)/n2**3, (2. - 3.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (2. - 3.*n2 + n2**2)/n2**2, (-4. + 4.*n2)/(n2*n3), (-1.*n3 + n2*n3)/(n2*n3), (-4. + 4.*n3)/(n1*n3), (-1.*n1 + n1*n3)/(n1*n3), (-1. + n3)/n3, (2. - 3.*n3 + n3**2)/n3**2, (-1. + n3)/n3, (-4. + 4.*n3)/(n2*n3), (-1.*n2 + n2*n3)/(n2*n3), (2. - 3.*n3 + n3**2)/n3**2, (2. - 3.*n3 + n3**2)/n3**2, (2. - 3.*n3 + n3**2)/n3**2, (8. - 12.*n3 + 4.*n3**2)/n3**3, (-4. + 8.*n3 - 5.*n3**2 + n3**3)/n3**3, (-2. + 2.*n1)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (n1 - 2.*n1**2 + n1**3)/n1**3, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (-1. + n1)/n1, (1. - 1.*n1 - 1.*n3 + n1*n3)/(n1*n3), (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, 1/(n1*n2), 0.25/n1, 0.25/n2, 1., 0.25/n1, 1., (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, 0.25/n2, 1., (-1. + n3)/n3, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, 0.25/n1, 1., 1/(n1*n3), 0.25/n1, 0.25/n3, 1., (-1. + n2)/n2, 0.25/n3, 1., (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (-2. + 2.*n2)/n2**3, (1. - 2.*n2 + n2**2)/n2**3, (n2 - 2.*n2**2 + n2**3)/n2**3, (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (1. - 1.*n2 - 1.*n3 + n2*n3)/(n2*n3), (-1. + n1)/n1, 0.25/n2, 1., 0.25/n3, 1., (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, 1/(n2*n3), 0.25/n2, 0.25/n3, 1., (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (1. - 1.*n1 - 1.*n3 + n1*n3)/(n1*n3), (-1. + n3)/n3, (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (1. - 1.*n2 - 1.*n3 + n2*n3)/(n2*n3), (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (-2. + 2.*n3)/n3**3, (1. - 2.*n3 + n3**2)/n3**3, (n3 - 2.*n3**2 + n3**3)/n3**3, (-1. + n1)/n1, 1., 1., (-1. + n2)/n2, 1., (-1. + n3)/n3, (-1. + n1)/n1, 1., 1., (-1. + n2)/n2, 1., (-1. + n3)/n3, 1 ]
    return csc_matrix((data,(row,col)),shape=(82,82))

def pi_corrections_4pop(ns):
    n1,n2,n3,n4 = ns
    row = [ 0, 0, 0, 1, 2, 3, 4, 4, 4, 5, 6, 7, 7, 7, 8, 9, 9, 9, 10, 10, 11, 12, 13, 14, 15, 15, 16, 17, 18, 19, 20, 20, 21, 22, 23, 24, 25, 25, 26, 26, 27, 28, 29, 30, 31, 31, 32, 33, 34, 35, 36, 36, 37, 38, 39, 40, 41, 41, 42, 42, 43, 44, 45, 46, 47, 47, 48, 49, 50, 51, 52, 52, 53, 54, 55, 56, 57, 57, 58, 58, 59, 60, 61, 62, 63, 63, 64, 65, 66, 67, 68, 68, 69, 70, 71, 72, 73, 73, 74, 74, 74, 75, 75, 76, 76, 77, 77, 78, 79, 80, 81, 82, 83, 84, 84, 85, 85, 85, 85, 86, 86, 87, 87, 88, 88, 89, 89, 90, 90, 91, 92, 93, 94, 94, 95, 95, 96, 96, 96, 96, 97, 97, 98, 99, 99, 100, 101, 101, 102, 102, 103, 104, 104, 105, 105, 106, 106, 107, 107, 107, 107, 108, 109, 110, 110, 111, 112, 112, 113, 113, 114, 115, 115, 116, 117, 118, 118, 118, 119, 119, 120, 120, 121, 122, 123, 124, 125, 125, 126, 126, 127, 128, 128, 129, 129, 129, 129, 130, 130, 131, 131, 132, 132, 133, 134, 135, 135, 136, 137, 137, 138, 138, 139, 139, 140, 140, 140, 140, 141, 142, 142, 143, 143, 144, 145, 146, 146, 147, 148, 149, 149, 150, 151, 151, 151, 152, 152, 153, 154, 155, 156, 156, 157, 157, 158, 159, 159, 160, 160, 161, 161, 162, 162, 162, 162, 163, 163, 164, 165, 166, 167, 167, 168, 169, 170, 170, 171, 172, 172, 173, 173, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194 ]
    col = [ 0, 10, 74, 1, 2, 3, 4, 31, 118, 5, 6, 7, 52, 151, 8, 9, 73, 173, 0, 10, 11, 12, 13, 14, 1, 15, 16, 17, 18, 19, 2, 20, 21, 22, 23, 24, 3, 25, 1, 26, 27, 28, 29, 30, 4, 31, 32, 33, 34, 35, 5, 36, 37, 38, 39, 40, 6, 41, 2, 42, 43, 44, 45, 46, 5, 47, 48, 49, 50, 51, 7, 52, 53, 54, 55, 56, 8, 57, 3, 58, 59, 60, 61, 62, 6, 63, 64, 65, 66, 67, 8, 68, 69, 70, 71, 72, 9, 73, 0, 10, 74, 11, 75, 12, 76, 13, 77, 78, 79, 80, 81, 82, 83, 14, 84, 1, 15, 26, 85, 16, 86, 17, 87, 27, 88, 28, 89, 29, 90, 91, 92, 93, 18, 94, 19, 95, 2, 20, 42, 96, 21, 97, 98, 43, 99, 100, 44, 101, 45, 102, 103, 22, 104, 23, 105, 24, 106, 3, 25, 58, 107, 108, 109, 59, 110, 111, 60, 112, 61, 113, 114, 30, 115, 116, 117, 4, 31, 118, 32, 119, 33, 120, 121, 122, 123, 124, 34, 125, 46, 126, 127, 35, 128, 5, 36, 47, 129, 37, 130, 48, 131, 49, 132, 133, 134, 38, 135, 136, 62, 137, 39, 138, 40, 139, 6, 41, 63, 140, 141, 64, 142, 65, 143, 144, 145, 50, 146, 147, 148, 51, 149, 150, 7, 52, 151, 53, 152, 153, 154, 155, 54, 156, 66, 157, 158, 55, 159, 67, 160, 56, 161, 8, 57, 68, 162, 69, 163, 164, 165, 166, 70, 167, 168, 169, 71, 170, 171, 72, 172, 9, 73, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194 ]
    data = [ (-2. + 4.*n1 - 3.*n1**2 + n1**3)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (-1.*n1 + n1**2)/n1**3, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (1. - 1.*n1 - 1.*n3 + n1*n3)/(n1*n3), (1. - 1.*n1 - 1.*n4 + n1*n4)/(n1*n4), (-2. + 4.*n2 - 3.*n2**2 + n2**3)/n2**3, (1. - 2.*n2 + n2**2)/n2**3, (-1.*n2 + n2**2)/n2**3, (1. - 1.*n2 - 1.*n3 + n2*n3)/(n2*n3), (1. - 1.*n2 - 1.*n4 + n2*n4)/(n2*n4), (-2. + 4.*n3 - 3.*n3**2 + n3**3)/n3**3, (1. - 2.*n3 + n3**2)/n3**3, (-1.*n3 + n3**2)/n3**3, (1. - 1.*n3 - 1.*n4 + n3*n4)/(n3*n4), (-2. + 4.*n4 - 3.*n4**2 + n4**3)/n4**3, (1. - 2.*n4 + n4**2)/n4**3, (-1.*n4 + n4**2)/n4**3, (8. - 12.*n1 + 4.*n1**2)/n1**3, (-4. + 8.*n1 - 5.*n1**2 + n1**3)/n1**3, (2. - 3.*n1 + n1**2)/n1**2, (2. - 3.*n1 + n1**2)/n1**2, (2. - 3.*n1 + n1**2)/n1**2, (2. - 3.*n1 + n1**2)/n1**2, (-4. + 4.*n1)/(n1*n2), (-1.*n2 + n1*n2)/(n1*n2), (-1. + n1)/n1, (-1. + n1)/n1, (2. - 3.*n1 + n1**2)/n1**2, (-1. + n1)/n1, (-4. + 4.*n1)/(n1*n3), (-1.*n3 + n1*n3)/(n1*n3), (-1. + n1)/n1, (2. - 3.*n1 + n1**2)/n1**2, (-1. + n1)/n1, (-1. + n1)/n1, (-4. + 4.*n1)/(n1*n4), (-1.*n4 + n1*n4)/(n1*n4), (-4. + 4.*n2)/(n1*n2), (-1.*n1 + n1*n2)/(n1*n2), (2. - 3.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (-1. + n2)/n2, (2. - 3.*n2 + n2**2)/n2**2, (8. - 12.*n2 + 4.*n2**2)/n2**3, (-4. + 8.*n2 - 5.*n2**2 + n2**3)/n2**3, (2. - 3.*n2 + n2**2)/n2**2, (2. - 3.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (2. - 3.*n2 + n2**2)/n2**2, (-4. + 4.*n2)/(n2*n3), (-1.*n3 + n2*n3)/(n2*n3), (-1. + n2)/n2, (-1. + n2)/n2, (2. - 3.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (-4. + 4.*n2)/(n2*n4), (-1.*n4 + n2*n4)/(n2*n4), (-4. + 4.*n3)/(n1*n3), (-1.*n1 + n1*n3)/(n1*n3), (-1. + n3)/n3, (2. - 3.*n3 + n3**2)/n3**2, (-1. + n3)/n3, (-1. + n3)/n3, (-4. + 4.*n3)/(n2*n3), (-1.*n2 + n2*n3)/(n2*n3), (2. - 3.*n3 + n3**2)/n3**2, (-1. + n3)/n3, (2. - 3.*n3 + n3**2)/n3**2, (2. - 3.*n3 + n3**2)/n3**2, (8. - 12.*n3 + 4.*n3**2)/n3**3, (-4. + 8.*n3 - 5.*n3**2 + n3**3)/n3**3, (2. - 3.*n3 + n3**2)/n3**2, (-1. + n3)/n3, (-1. + n3)/n3, (2. - 3.*n3 + n3**2)/n3**2, (-4. + 4.*n3)/(n3*n4), (-1.*n4 + n3*n4)/(n3*n4), (-4. + 4.*n4)/(n1*n4), (-1.*n1 + n1*n4)/(n1*n4), (-1. + n4)/n4, (-1. + n4)/n4, (2. - 3.*n4 + n4**2)/n4**2, (-1. + n4)/n4, (-4. + 4.*n4)/(n2*n4), (-1.*n2 + n2*n4)/(n2*n4), (-1. + n4)/n4, (2. - 3.*n4 + n4**2)/n4**2, (-1. + n4)/n4, (-1. + n4)/n4, (-4. + 4.*n4)/(n3*n4), (-1.*n3 + n3*n4)/(n3*n4), (2. - 3.*n4 + n4**2)/n4**2, (2. - 3.*n4 + n4**2)/n4**2, (2. - 3.*n4 + n4**2)/n4**2, (2. - 3.*n4 + n4**2)/n4**2, (8. - 12.*n4 + 4.*n4**2)/n4**3, (-4. + 8.*n4 - 5.*n4**2 + n4**3)/n4**3, (-2. + 2.*n1)/n1**3, (1. - 2.*n1 + n1**2)/n1**3, (n1 - 2.*n1**2 + n1**3)/n1**3, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (-1. + n1)/n1, (-1. + n1)/n1, (1. - 1.*n1 - 1.*n3 + n1*n3)/(n1*n3), (-1. + n1)/n1, (1. - 1.*n1 - 1.*n4 + n1*n4)/(n1*n4), (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, 1/(n1*n2), 0.25/n1, 0.25/n2, 1., 0.25/n1, 1., 0.25/n1, 1., (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, 0.25/n2, 1., 0.25/n2, 1., (-1. + n3)/n3, 1., (-1. + n4)/n4, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, 0.25/n1, 1., 1/(n1*n3), 0.25/n1, 0.25/n3, 1., 0.25/n1, 1., (-1. + n2)/n2, 0.25/n3, 1., 1., (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, 0.25/n3, 1., (-1. + n4)/n4, (0.5*(-1. + n1))/n1**2, (-1.*n1 + n1**2)/n1**2, 0.25/n1, 1., 0.25/n1, 1., 1/(n1*n4), 0.25/n1, 0.25/n4, 1., (-1. + n2)/n2, 1., 0.25/n4, 1., (-1. + n3)/n3, 0.25/n4, 1., (0.5*(-1. + n4))/n4**2, (-1.*n4 + n4**2)/n4**2, (1. - 1.*n1 - 1.*n2 + n1*n2)/(n1*n2), (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (-1. + n2)/n2, (-1. + n2)/n2, (-2. + 2.*n2)/n2**3, (1. - 2.*n2 + n2**2)/n2**3, (n2 - 2.*n2**2 + n2**3)/n2**3, (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, (1. - 1.*n2 - 1.*n3 + n2*n3)/(n2*n3), (-1. + n2)/n2, (1. - 1.*n2 - 1.*n4 + n2*n4)/(n2*n4), (-1. + n1)/n1, 0.25/n2, 1., 0.25/n3, 1., 1., (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, 1/(n2*n3), 0.25/n2, 0.25/n3, 1., 0.25/n2, 1., (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, 0.25/n3, 1., (-1. + n4)/n4, (-1. + n1)/n1, 0.25/n2, 1., 1., 0.25/n4, 1., (0.5*(-1. + n2))/n2**2, (-1.*n2 + n2**2)/n2**2, 0.25/n2, 1., 1/(n2*n4), 0.25/n2, 0.25/n4, 1., (-1. + n3)/n3, 0.25/n4, 1., (0.5*(-1. + n4))/n4**2, (-1.*n4 + n4**2)/n4**2, (1. - 1.*n1 - 1.*n3 + n1*n3)/(n1*n3), (-1. + n3)/n3, (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (-1. + n3)/n3, (1. - 1.*n2 - 1.*n3 + n2*n3)/(n2*n3), (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (-1. + n3)/n3, (-2. + 2.*n3)/n3**3, (1. - 2.*n3 + n3**2)/n3**3, (n3 - 2.*n3**2 + n3**3)/n3**3, (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, (1. - 1.*n3 - 1.*n4 + n3*n4)/(n3*n4), (-1. + n1)/n1, 1., 0.25/n3, 1., 0.25/n4, 1., (-1. + n2)/n2, 0.25/n3, 1., 0.25/n4, 1., (0.5*(-1. + n3))/n3**2, (-1.*n3 + n3**2)/n3**2, 1/(n3*n4), 0.25/n3, 0.25/n4, 1., (0.5*(-1. + n4))/n4**2, (-1.*n4 + n4**2)/n4**2, (1. - 1.*n1 - 1.*n4 + n1*n4)/(n1*n4), (-1. + n4)/n4, (-1. + n4)/n4, (0.5*(-1. + n4))/n4**2, (-1.*n4 + n4**2)/n4**2, (1. - 1.*n2 - 1.*n4 + n2*n4)/(n2*n4), (-1. + n4)/n4, (0.5*(-1. + n4))/n4**2, (-1.*n4 + n4**2)/n4**2, (1. - 1.*n3 - 1.*n4 + n3*n4)/(n3*n4), (0.5*(-1. + n4))/n4**2, (-1.*n4 + n4**2)/n4**2, (-2. + 2.*n4)/n4**3, (1. - 2.*n4 + n4**2)/n4**3, (n4 - 2.*n4**2 + n4**3)/n4**3, (-1. + n1)/n1, 1., 1., 1., (-1. + n2)/n2, 1., 1., (-1. + n3)/n3, 1., (-1. + n4)/n4, (-1. + n1)/n1, 1., 1., 1., (-1. + n2)/n2, 1., 1., (-1. + n3)/n3, 1., (-1. + n4)/n4, 1 ]
    return csc_matrix((data,(row,col)),shape=(195,195))
    
def pi_corrections_genotype_1pop(ns):
    n1 = ns[0]
    row = [ 0, 0, 0, 1, 1, 2, 2, 2, 3, 4, 5 ]
    col = [ 0, 1, 2, 0, 1, 0, 1, 2, 3, 4, 5 ]
    data = [ (0.125*(-2. + 4.*n1 - 4.*n1**2 + 2.*n1**3))/n1**3, (0.125*(1. - 2.*n1 + n1**2))/n1**3, (0.125*(-2.*n1 + 2.*n1**2))/n1**3, (0.5*(2. - 4.*n1 + 2.*n1**2))/n1**3, (0.5*(-1. + 3.*n1 - 3.*n1**2 + n1**3))/n1**3, (0.125*(-2. + 4.*n1))/n1**3, (0.125*(1. - 4.*n1 + 4.*n1**2))/n1**3, (0.125*(2.*n1 - 8.*n1**2 + 8.*n1**3))/n1**3, (0.5*(-1. + 2.*n1))/n1, (0.5*(-1. + 2.*n1))/n1, 1 ]
    return csc_matrix((data,(row,col)),shape=(6,6))

def pi_corrections_genotype_2pop(ns):
    n1,n2 = ns
    row = [ 0, 0, 0, 1, 2, 2, 2, 3, 3, 4, 5, 6, 6, 7, 7, 8, 9, 10, 10, 11, 11, 11, 12, 12, 13, 14, 14, 15, 15, 15, 15, 16, 16, 17, 18, 18, 19, 19, 19, 20, 21, 22, 23, 24, 25, 26 ]
    col = [ 0, 3, 11, 1, 2, 10, 19, 0, 3, 4, 5, 1, 6, 1, 7, 8, 9, 2, 10, 0, 3, 11, 4, 12, 13, 5, 14, 1, 6, 7, 15, 8, 16, 17, 9, 18, 2, 10, 19, 20, 21, 22, 23, 24, 25, 26 ]
    data = [ (0.125*(-2. + 4.*n1 - 4.*n1**2 + 2.*n1**3))/n1**3, (0.125*(1. - 2.*n1 + n1**2))/n1**3, (0.125*(-2.*n1 + 2.*n1**2))/n1**3, (0.25*(1. - 1.*n1 - 1.*n2 + n1*n2))/(n1*n2), (0.125*(-2. + 4.*n2 - 4.*n2**2 + 2.*n2**3))/n2**3, (0.125*(1. - 2.*n2 + n2**2))/n2**3, (0.125*(-2.*n2 + 2.*n2**2))/n2**3, (0.5*(2. - 4.*n1 + 2.*n1**2))/n1**3, (0.5*(-1. + 3.*n1 - 3.*n1**2 + n1**3))/n1**3, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(-2. + 2.*n1))/(n1*n2), (0.5*(-1.*n2 + n1*n2))/(n1*n2), (0.5*(-2. + 2.*n2))/(n1*n2), (0.5*(-1.*n1 + n1*n2))/(n1*n2), (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(2. - 4.*n2 + 2.*n2**2))/n2**3, (0.5*(-1. + 3.*n2 - 3.*n2**2 + n2**3))/n2**3, (0.125*(-2. + 4.*n1))/n1**3, (0.125*(1. - 4.*n1 + 4.*n1**2))/n1**3, (0.125*(2.*n1 - 8.*n1**2 + 8.*n1**3))/n1**3, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, (0.25*(1. - 2.*n1 - 2.*n2 + 4.*n1*n2))/(n1*n2), (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, 0.25/(n1*n2), 0.125/n1, 0.125/n2, 1., (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.25*(1. - 2.*n1 - 2.*n2 + 4.*n1*n2))/(n1*n2), (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.125*(-2. + 4.*n2))/n2**3, (0.125*(1. - 4.*n2 + 4.*n2**2))/n2**3, (0.125*(2.*n2 - 8.*n2**2 + 8.*n2**3))/n2**3, (0.5*(-1. + 2.*n1))/n1, 1., (0.5*(-1. + 2.*n2))/n2, (0.5*(-1. + 2.*n1))/n1, 1., (0.5*(-1. + 2.*n2))/n2, 1 ]
    return csc_matrix((data,(row,col)),shape=(27,27))

def pi_corrections_genotype_3pop(ns):
    n1,n2,n3 = ns
    row = [ 0, 0, 0, 1, 2, 3, 3, 3, 4, 5, 5, 5, 6, 6, 7, 8, 9, 10, 10, 11, 12, 13, 14, 14, 15, 15, 16, 17, 18, 19, 19, 20, 21, 22, 23, 23, 24, 24, 25, 26, 27, 28, 28, 29, 30, 31, 32, 32, 33, 33, 33, 34, 34, 35, 35, 36, 37, 38, 39, 39, 40, 40, 40, 40, 41, 41, 42, 42, 43, 43, 44, 45, 45, 46, 46, 47, 47, 47, 47, 48, 49, 49, 50, 50, 51, 52, 52, 53, 54, 54, 54, 55, 55, 56, 57, 58, 58, 59, 59, 60, 60, 61, 61, 61, 61, 62, 62, 63, 64, 65, 65, 66, 67, 67, 68, 68, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81 ]
    col = [ 0, 6, 33, 1, 2, 3, 19, 54, 4, 5, 32, 68, 0, 6, 7, 8, 9, 1, 10, 11, 12, 13, 2, 14, 1, 15, 16, 17, 18, 3, 19, 20, 21, 22, 4, 23, 2, 24, 25, 26, 27, 4, 28, 29, 30, 31, 5, 32, 0, 6, 33, 7, 34, 8, 35, 36, 37, 38, 9, 39, 1, 10, 15, 40, 11, 41, 16, 42, 17, 43, 44, 12, 45, 13, 46, 2, 14, 24, 47, 48, 25, 49, 26, 50, 51, 18, 52, 53, 3, 19, 54, 20, 55, 56, 57, 21, 58, 27, 59, 22, 60, 4, 23, 28, 61, 29, 62, 63, 64, 30, 65, 66, 31, 67, 5, 32, 68, 69, 70, 71, 72, 73, 74, 75, 76, 77, 78, 79, 80, 81 ]
    data = [ (0.125*(-2. + 4.*n1 - 4.*n1**2 + 2.*n1**3))/n1**3, (0.125*(1. - 2.*n1 + n1**2))/n1**3, (0.125*(-2.*n1 + 2.*n1**2))/n1**3, (0.25*(1. - 1.*n1 - 1.*n2 + n1*n2))/(n1*n2), (0.25*(1. - 1.*n1 - 1.*n3 + n1*n3))/(n1*n3), (0.125*(-2. + 4.*n2 - 4.*n2**2 + 2.*n2**3))/n2**3, (0.125*(1. - 2.*n2 + n2**2))/n2**3, (0.125*(-2.*n2 + 2.*n2**2))/n2**3, (0.25*(1. - 1.*n2 - 1.*n3 + n2*n3))/(n2*n3), (0.125*(-2. + 4.*n3 - 4.*n3**2 + 2.*n3**3))/n3**3, (0.125*(1. - 2.*n3 + n3**2))/n3**3, (0.125*(-2.*n3 + 2.*n3**2))/n3**3, (0.5*(2. - 4.*n1 + 2.*n1**2))/n1**3, (0.5*(-1. + 3.*n1 - 3.*n1**2 + n1**3))/n1**3, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(-2. + 2.*n1))/(n1*n2), (0.5*(-1.*n2 + n1*n2))/(n1*n2), (0.5*(-1. + n1))/n1, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(-1. + n1))/n1, (0.5*(-2. + 2.*n1))/(n1*n3), (0.5*(-1.*n3 + n1*n3))/(n1*n3), (0.5*(-2. + 2.*n2))/(n1*n2), (0.5*(-1.*n1 + n1*n2))/(n1*n2), (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-1. + n2))/n2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(2. - 4.*n2 + 2.*n2**2))/n2**3, (0.5*(-1. + 3.*n2 - 3.*n2**2 + n2**3))/n2**3, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-1. + n2))/n2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-2. + 2.*n2))/(n2*n3), (0.5*(-1.*n3 + n2*n3))/(n2*n3), (0.5*(-2. + 2.*n3))/(n1*n3), (0.5*(-1.*n1 + n1*n3))/(n1*n3), (0.5*(-1. + n3))/n3, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(-1. + n3))/n3, (0.5*(-2. + 2.*n3))/(n2*n3), (0.5*(-1.*n2 + n2*n3))/(n2*n3), (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(2. - 4.*n3 + 2.*n3**2))/n3**3, (0.5*(-1. + 3.*n3 - 3.*n3**2 + n3**3))/n3**3, (0.125*(-2. + 4.*n1))/n1**3, (0.125*(1. - 4.*n1 + 4.*n1**2))/n1**3, (0.125*(2.*n1 - 8.*n1**2 + 8.*n1**3))/n1**3, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, (0.25*(1. - 2.*n1 - 2.*n2 + 4.*n1*n2))/(n1*n2), (0.5*(-1. + 2.*n1))/n1, (0.25*(1. - 2.*n1 - 2.*n3 + 4.*n1*n3))/(n1*n3), (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, 0.25/(n1*n2), 0.125/n1, 0.125/n2, 1., 0.125/n1, 1., (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, 0.125/n2, 1., (0.5*(-1. + 2.*n3))/n3, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, 0.125/n1, 1., 0.25/(n1*n3), 0.125/n1, 0.125/n3, 1., (0.5*(-1. + 2.*n2))/n2, 0.125/n3, 1., (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.25*(1. - 2.*n1 - 2.*n2 + 4.*n1*n2))/(n1*n2), (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.5*(-1. + 2.*n2))/n2, (0.125*(-2. + 4.*n2))/n2**3, (0.125*(1. - 4.*n2 + 4.*n2**2))/n2**3, (0.125*(2.*n2 - 8.*n2**2 + 8.*n2**3))/n2**3, (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.25*(1. - 2.*n2 - 2.*n3 + 4.*n2*n3))/(n2*n3), (0.5*(-1. + 2.*n1))/n1, 0.125/n2, 1., 0.125/n3, 1., (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, 0.25/(n2*n3), 0.125/n2, 0.125/n3, 1., (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.25*(1. - 2.*n1 - 2.*n3 + 4.*n1*n3))/(n1*n3), (0.5*(-1. + 2.*n3))/n3, (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.25*(1. - 2.*n2 - 2.*n3 + 4.*n2*n3))/(n2*n3), (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.125*(-2. + 4.*n3))/n3**3, (0.125*(1. - 4.*n3 + 4.*n3**2))/n3**3, (0.125*(2.*n3 - 8.*n3**2 + 8.*n3**3))/n3**3, (0.5*(-1. + 2.*n1))/n1, 1., 1., (0.5*(-1. + 2.*n2))/n2, 1., (0.5*(-1. + 2.*n3))/n3, (0.5*(-1. + 2.*n1))/n1, 1., 1., (0.5*(-1. + 2.*n2))/n2, 1., (0.5*(-1. + 2.*n3))/n3, 1 ]
    return csc_matrix((data,(row,col)),shape=(82,82))

def pi_corrections_genotype_4pop(ns):
    n1,n2,n3,n4 = ns
    row = [ 0, 0, 0, 1, 2, 3, 4, 4, 4, 5, 6, 7, 7, 7, 8, 9, 9, 9, 10, 10, 11, 12, 13, 14, 15, 15, 16, 17, 18, 19, 20, 20, 21, 22, 23, 24, 25, 25, 26, 26, 27, 28, 29, 30, 31, 31, 32, 33, 34, 35, 36, 36, 37, 38, 39, 40, 41, 41, 42, 42, 43, 44, 45, 46, 47, 47, 48, 49, 50, 51, 52, 52, 53, 54, 55, 56, 57, 57, 58, 58, 59, 60, 61, 62, 63, 63, 64, 65, 66, 67, 68, 68, 69, 70, 71, 72, 73, 73, 74, 74, 74, 75, 75, 76, 76, 77, 77, 78, 79, 80, 81, 82, 83, 84, 84, 85, 85, 85, 85, 86, 86, 87, 87, 88, 88, 89, 89, 90, 90, 91, 92, 93, 94, 94, 95, 95, 96, 96, 96, 96, 97, 97, 98, 99, 99, 100, 101, 101, 102, 102, 103, 104, 104, 105, 105, 106, 106, 107, 107, 107, 107, 108, 109, 110, 110, 111, 112, 112, 113, 113, 114, 115, 115, 116, 117, 118, 118, 118, 119, 119, 120, 120, 121, 122, 123, 124, 125, 125, 126, 126, 127, 128, 128, 129, 129, 129, 129, 130, 130, 131, 131, 132, 132, 133, 134, 135, 135, 136, 137, 137, 138, 138, 139, 139, 140, 140, 140, 140, 141, 142, 142, 143, 143, 144, 145, 146, 146, 147, 148, 149, 149, 150, 151, 151, 151, 152, 152, 153, 154, 155, 156, 156, 157, 157, 158, 159, 159, 160, 160, 161, 161, 162, 162, 162, 162, 163, 163, 164, 165, 166, 167, 167, 168, 169, 170, 170, 171, 172, 172, 173, 173, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194 ]
    col = [ 0, 10, 74, 1, 2, 3, 4, 31, 118, 5, 6, 7, 52, 151, 8, 9, 73, 173, 0, 10, 11, 12, 13, 14, 1, 15, 16, 17, 18, 19, 2, 20, 21, 22, 23, 24, 3, 25, 1, 26, 27, 28, 29, 30, 4, 31, 32, 33, 34, 35, 5, 36, 37, 38, 39, 40, 6, 41, 2, 42, 43, 44, 45, 46, 5, 47, 48, 49, 50, 51, 7, 52, 53, 54, 55, 56, 8, 57, 3, 58, 59, 60, 61, 62, 6, 63, 64, 65, 66, 67, 8, 68, 69, 70, 71, 72, 9, 73, 0, 10, 74, 11, 75, 12, 76, 13, 77, 78, 79, 80, 81, 82, 83, 14, 84, 1, 15, 26, 85, 16, 86, 17, 87, 27, 88, 28, 89, 29, 90, 91, 92, 93, 18, 94, 19, 95, 2, 20, 42, 96, 21, 97, 98, 43, 99, 100, 44, 101, 45, 102, 103, 22, 104, 23, 105, 24, 106, 3, 25, 58, 107, 108, 109, 59, 110, 111, 60, 112, 61, 113, 114, 30, 115, 116, 117, 4, 31, 118, 32, 119, 33, 120, 121, 122, 123, 124, 34, 125, 46, 126, 127, 35, 128, 5, 36, 47, 129, 37, 130, 48, 131, 49, 132, 133, 134, 38, 135, 136, 62, 137, 39, 138, 40, 139, 6, 41, 63, 140, 141, 64, 142, 65, 143, 144, 145, 50, 146, 147, 148, 51, 149, 150, 7, 52, 151, 53, 152, 153, 154, 155, 54, 156, 66, 157, 158, 55, 159, 67, 160, 56, 161, 8, 57, 68, 162, 69, 163, 164, 165, 166, 70, 167, 168, 169, 71, 170, 171, 72, 172, 9, 73, 173, 174, 175, 176, 177, 178, 179, 180, 181, 182, 183, 184, 185, 186, 187, 188, 189, 190, 191, 192, 193, 194 ]
    data = [ (0.125*(-2. + 4.*n1 - 4.*n1**2 + 2.*n1**3))/n1**3, (0.125*(1. - 2.*n1 + n1**2))/n1**3, (0.125*(-2.*n1 + 2.*n1**2))/n1**3, (0.25*(1. - 1.*n1 - 1.*n2 + n1*n2))/(n1*n2), (0.25*(1. - 1.*n1 - 1.*n3 + n1*n3))/(n1*n3), (0.25*(1. - 1.*n1 - 1.*n4 + n1*n4))/(n1*n4), (0.125*(-2. + 4.*n2 - 4.*n2**2 + 2.*n2**3))/n2**3, (0.125*(1. - 2.*n2 + n2**2))/n2**3, (0.125*(-2.*n2 + 2.*n2**2))/n2**3, (0.25*(1. - 1.*n2 - 1.*n3 + n2*n3))/(n2*n3), (0.25*(1. - 1.*n2 - 1.*n4 + n2*n4))/(n2*n4), (0.125*(-2. + 4.*n3 - 4.*n3**2 + 2.*n3**3))/n3**3, (0.125*(1. - 2.*n3 + n3**2))/n3**3, (0.125*(-2.*n3 + 2.*n3**2))/n3**3, (0.25*(1. - 1.*n3 - 1.*n4 + n3*n4))/(n3*n4), (0.125*(-2. + 4.*n4 - 4.*n4**2 + 2.*n4**3))/n4**3, (0.125*(1. - 2.*n4 + n4**2))/n4**3, (0.125*(-2.*n4 + 2.*n4**2))/n4**3, (0.5*(2. - 4.*n1 + 2.*n1**2))/n1**3, (0.5*(-1. + 3.*n1 - 3.*n1**2 + n1**3))/n1**3, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(-2. + 2.*n1))/(n1*n2), (0.5*(-1.*n2 + n1*n2))/(n1*n2), (0.5*(-1. + n1))/n1, (0.5*(-1. + n1))/n1, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(-1. + n1))/n1, (0.5*(-2. + 2.*n1))/(n1*n3), (0.5*(-1.*n3 + n1*n3))/(n1*n3), (0.5*(-1. + n1))/n1, (0.5*(1. - 2.*n1 + n1**2))/n1**2, (0.5*(-1. + n1))/n1, (0.5*(-1. + n1))/n1, (0.5*(-2. + 2.*n1))/(n1*n4), (0.5*(-1.*n4 + n1*n4))/(n1*n4), (0.5*(-2. + 2.*n2))/(n1*n2), (0.5*(-1.*n1 + n1*n2))/(n1*n2), (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-1. + n2))/n2, (0.5*(-1. + n2))/n2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(2. - 4.*n2 + 2.*n2**2))/n2**3, (0.5*(-1. + 3.*n2 - 3.*n2**2 + n2**3))/n2**3, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-1. + n2))/n2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-2. + 2.*n2))/(n2*n3), (0.5*(-1.*n3 + n2*n3))/(n2*n3), (0.5*(-1. + n2))/n2, (0.5*(-1. + n2))/n2, (0.5*(1. - 2.*n2 + n2**2))/n2**2, (0.5*(-1. + n2))/n2, (0.5*(-2. + 2.*n2))/(n2*n4), (0.5*(-1.*n4 + n2*n4))/(n2*n4), (0.5*(-2. + 2.*n3))/(n1*n3), (0.5*(-1.*n1 + n1*n3))/(n1*n3), (0.5*(-1. + n3))/n3, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(-1. + n3))/n3, (0.5*(-1. + n3))/n3, (0.5*(-2. + 2.*n3))/(n2*n3), (0.5*(-1.*n2 + n2*n3))/(n2*n3), (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(-1. + n3))/n3, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(2. - 4.*n3 + 2.*n3**2))/n3**3, (0.5*(-1. + 3.*n3 - 3.*n3**2 + n3**3))/n3**3, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(-1. + n3))/n3, (0.5*(-1. + n3))/n3, (0.5*(1. - 2.*n3 + n3**2))/n3**2, (0.5*(-2. + 2.*n3))/(n3*n4), (0.5*(-1.*n4 + n3*n4))/(n3*n4), (0.5*(-2. + 2.*n4))/(n1*n4), (0.5*(-1.*n1 + n1*n4))/(n1*n4), (0.5*(-1. + n4))/n4, (0.5*(-1. + n4))/n4, (0.5*(1. - 2.*n4 + n4**2))/n4**2, (0.5*(-1. + n4))/n4, (0.5*(-2. + 2.*n4))/(n2*n4), (0.5*(-1.*n2 + n2*n4))/(n2*n4), (0.5*(-1. + n4))/n4, (0.5*(1. - 2.*n4 + n4**2))/n4**2, (0.5*(-1. + n4))/n4, (0.5*(-1. + n4))/n4, (0.5*(-2. + 2.*n4))/(n3*n4), (0.5*(-1.*n3 + n3*n4))/(n3*n4), (0.5*(1. - 2.*n4 + n4**2))/n4**2, (0.5*(1. - 2.*n4 + n4**2))/n4**2, (0.5*(1. - 2.*n4 + n4**2))/n4**2, (0.5*(1. - 2.*n4 + n4**2))/n4**2, (0.5*(2. - 4.*n4 + 2.*n4**2))/n4**3, (0.5*(-1. + 3.*n4 - 3.*n4**2 + n4**3))/n4**3, (0.125*(-2. + 4.*n1))/n1**3, (0.125*(1. - 4.*n1 + 4.*n1**2))/n1**3, (0.125*(2.*n1 - 8.*n1**2 + 8.*n1**3))/n1**3, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, (0.25*(1. - 2.*n1 - 2.*n2 + 4.*n1*n2))/(n1*n2), (0.5*(-1. + 2.*n1))/n1, (0.5*(-1. + 2.*n1))/n1, (0.25*(1. - 2.*n1 - 2.*n3 + 4.*n1*n3))/(n1*n3), (0.5*(-1. + 2.*n1))/n1, (0.25*(1. - 2.*n1 - 2.*n4 + 4.*n1*n4))/(n1*n4), (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, 0.25/(n1*n2), 0.125/n1, 0.125/n2, 1., 0.125/n1, 1., 0.125/n1, 1., (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, 0.125/n2, 1., 0.125/n2, 1., (0.5*(-1. + 2.*n3))/n3, 1., (0.5*(-1. + 2.*n4))/n4, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, 0.125/n1, 1., 0.25/(n1*n3), 0.125/n1, 0.125/n3, 1., 0.125/n1, 1., (0.5*(-1. + 2.*n2))/n2, 0.125/n3, 1., 1., (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, 0.125/n3, 1., (0.5*(-1. + 2.*n4))/n4, (0.125*(-1. + 2.*n1))/n1**2, (0.25*(-2.*n1 + 4.*n1**2))/n1**2, 0.125/n1, 1., 0.125/n1, 1., 0.25/(n1*n4), 0.125/n1, 0.125/n4, 1., (0.5*(-1. + 2.*n2))/n2, 1., 0.125/n4, 1., (0.5*(-1. + 2.*n3))/n3, 0.125/n4, 1., (0.125*(-1. + 2.*n4))/n4**2, (0.25*(-2.*n4 + 4.*n4**2))/n4**2, (0.25*(1. - 2.*n1 - 2.*n2 + 4.*n1*n2))/(n1*n2), (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.5*(-1. + 2.*n2))/n2, (0.5*(-1. + 2.*n2))/n2, (0.125*(-2. + 4.*n2))/n2**3, (0.125*(1. - 4.*n2 + 4.*n2**2))/n2**3, (0.125*(2.*n2 - 8.*n2**2 + 8.*n2**3))/n2**3, (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, (0.25*(1. - 2.*n2 - 2.*n3 + 4.*n2*n3))/(n2*n3), (0.5*(-1. + 2.*n2))/n2, (0.25*(1. - 2.*n2 - 2.*n4 + 4.*n2*n4))/(n2*n4), (0.5*(-1. + 2.*n1))/n1, 0.125/n2, 1., 0.125/n3, 1., 1., (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, 0.25/(n2*n3), 0.125/n2, 0.125/n3, 1., 0.125/n2, 1., (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, 0.125/n3, 1., (0.5*(-1. + 2.*n4))/n4, (0.5*(-1. + 2.*n1))/n1, 0.125/n2, 1., 1., 0.125/n4, 1., (0.125*(-1. + 2.*n2))/n2**2, (0.25*(-2.*n2 + 4.*n2**2))/n2**2, 0.125/n2, 1., 0.25/(n2*n4), 0.125/n2, 0.125/n4, 1., (0.5*(-1. + 2.*n3))/n3, 0.125/n4, 1., (0.125*(-1. + 2.*n4))/n4**2, (0.25*(-2.*n4 + 4.*n4**2))/n4**2, (0.25*(1. - 2.*n1 - 2.*n3 + 4.*n1*n3))/(n1*n3), (0.5*(-1. + 2.*n3))/n3, (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.5*(-1. + 2.*n3))/n3, (0.25*(1. - 2.*n2 - 2.*n3 + 4.*n2*n3))/(n2*n3), (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.5*(-1. + 2.*n3))/n3, (0.125*(-2. + 4.*n3))/n3**3, (0.125*(1. - 4.*n3 + 4.*n3**2))/n3**3, (0.125*(2.*n3 - 8.*n3**2 + 8.*n3**3))/n3**3, (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, (0.25*(1. - 2.*n3 - 2.*n4 + 4.*n3*n4))/(n3*n4), (0.5*(-1. + 2.*n1))/n1, 1., 0.125/n3, 1., 0.125/n4, 1., (0.5*(-1. + 2.*n2))/n2, 0.125/n3, 1., 0.125/n4, 1., (0.125*(-1. + 2.*n3))/n3**2, (0.25*(-2.*n3 + 4.*n3**2))/n3**2, 0.25/(n3*n4), 0.125/n3, 0.125/n4, 1., (0.125*(-1. + 2.*n4))/n4**2, (0.25*(-2.*n4 + 4.*n4**2))/n4**2, (0.25*(1. - 2.*n1 - 2.*n4 + 4.*n1*n4))/(n1*n4), (0.5*(-1. + 2.*n4))/n4, (0.5*(-1. + 2.*n4))/n4, (0.125*(-1. + 2.*n4))/n4**2, (0.25*(-2.*n4 + 4.*n4**2))/n4**2, (0.25*(1. - 2.*n2 - 2.*n4 + 4.*n2*n4))/(n2*n4), (0.5*(-1. + 2.*n4))/n4, (0.125*(-1. + 2.*n4))/n4**2, (0.25*(-2.*n4 + 4.*n4**2))/n4**2, (0.25*(1. - 2.*n3 - 2.*n4 + 4.*n3*n4))/(n3*n4), (0.125*(-1. + 2.*n4))/n4**2, (0.25*(-2.*n4 + 4.*n4**2))/n4**2, (0.125*(-2. + 4.*n4))/n4**3, (0.125*(1. - 4.*n4 + 4.*n4**2))/n4**3, (0.125*(2.*n4 - 8.*n4**2 + 8.*n4**3))/n4**3, (0.5*(-1. + 2.*n1))/n1, 1., 1., 1., (0.5*(-1. + 2.*n2))/n2, 1., 1., (0.5*(-1. + 2.*n3))/n3, 1., (0.5*(-1. + 2.*n4))/n4, (0.5*(-1. + 2.*n1))/n1, 1., 1., 1., (0.5*(-1. + 2.*n2))/n2, 1., 1., (0.5*(-1. + 2.*n3))/n3, 1., (0.5*(-1. + 2.*n4))/n4, 1 ]
    return csc_matrix((data,(row,col)),shape=(195,195))
