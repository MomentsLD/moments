...
def assign_recombination_rates(
...
def get_reported_stats(
    ...
    if use_cache == True:
        type_counts = count_types_sparse(
            genotypes,
            bins,
            sample_ids,
            positions=positions,
...
def compute_ld_statistics(
    ...
    if rec_map_file is not None and r_bins is not None:
        if report is True:
            ...
            sys.stdout.flush()
        pos_rs = assign_recombination_rates(
            positions,
            rec_map_file,
            map_name=map_name,
            map_sep=map_sep,
    ...
    reported_stats = get_reported_stats(
        genotypes,
        bins,
        sample_ids,
        positions=positions,
