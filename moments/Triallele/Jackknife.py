...
def closest_ij_1(i,j,n):
    ...
    smallests = np.argpartition(np.sum((np.array([fi,fj]) - possible_ij/(1.*n))**2,axis=1),6)[:6]
    ...
    while i_range < 2 or j_range < 2:
        smallests = np.argpartition(np.sum((np.array([fi,fj]) - possible_ij/(1.*n))**2,axis=1),next_index)[:next_index]
        smallest_set = np.array([possible_ij[k] for k in smallests])
        distances = np.sum((np.array(smallest_set)/float(n) - [fi,fj])**2,axis=1)
        order = distances.argsort()
...
def compute_alphas_2(i,j,ordered_set,n):
    ...
    A[5] = (ordered_set[:,1]+1.)*(ordered_set[:,1]+2.)
    b[0] = (n+1.)*(n+2.)/((n+3.)*(n+4.))
    b[1] = (n+1.)*(n+2.)/((n+4.)*(n+5.))*(i+1.)
    b[2] = (n+1.)*(n+2.)/((n+4.)*(n+5.))*(j+1.)
    b[3] = (n+1.)*(n+2.)/((n+5.)*(n+6.))*(i+1.)*(i+2.)
    b[4] = (n+1.)*(n+2.)/((n+5.)*(n+6.))*(i+1.)*(j+1.)
    b[5] = (n+1.)*(n+2.)/((n+5.)*(n+6.))*(j+1.)*(j+2.)    
    return np.dot(np.linalg.inv(A),b)
...
def find_iprime_1D(n,i):
    ...
    iis = np.arange(n+1)
    ii = np.argmin(abs(iis/(1.*n) - i/(n+2.)))
    if ii < 2: ii = 2
    if ii > n-2: ii = n-2
    return ii
